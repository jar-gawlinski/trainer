...
Create a new GitHub repo (e.g. `train`) and upload all files in this folder to the root of the repo (`index.html`, `manifest.json`, `sw.js`, `icon.svg`, `icon-192.png`, `icon-512.png`, `apple-touch-icon.png`).
In the repo, go to Settings → Pages.
Under "Build and deployment", set Source to `Deploy from a branch`, branch `main`, folder `/ (root)`. Save.
Wait a minute, then your app will be live at `https://<your-username>.github.io/<repo-name>/`.
...
Tap the ⋮ menu → Add to Home screen (or you may get an automatic "Install app" prompt).
